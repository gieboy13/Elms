...
if ($status == 1){
    ...
    if($query->rowCount() > 0)
    {
        ...
        {      
            $lt=htmlentities($result->LeaveType);
            $empid=htmlentities($result->empid);
        }
       
        if ($lt == "Sick Leave"){
         
             $sql = "UPDATE empcredits SET sick_credits = (sick_credits - 1) WHERE empid=".$empid;

             $query = $dbh->prepare($sql);
            $query->execute();
       }

       else if ($lt == "Vacation Leave"){
            $sql = "UPDATE empcredits SET vacation_credits = (vacation_credits - 1) WHERE empid=".$empid;
                                        ...
                                        <tr>
                                          ...
                                          <tr>
                                            ...
                                            <td><?php echo htmlentities($result->Phonenumber);?></td>
                                             <td style="font-size:16px;"><b>Employee Code :</b></td>
